...
function getPlatformEra(p: Platform): string {
  ...
  if (
    name.includes("playdate") ||
    name.includes("quest 3") ||
    name.includes("ps5") ||
...
function EraGroup({
  ...
  items: Platform[];
  navigate: any;
  isYourSystems?: boolean;
  userSystemCounts?: Map<string, number>;
}) {
  ...
  const sorted = useMemo(() => {
    if (isYourSystems && userSystemCounts) {
      return [...items].sort((a, b) => {
        const countA =
          userSystemCounts.get(a.name) ||
          (a.abbreviation ? userSystemCounts.get(a.abbreviation) : 0) ||
          0;
        const countB =
          userSystemCounts.get(b.name) ||
          (b.abbreviation ? userSystemCounts.get(b.abbreviation) : 0) ||
          0;
  ...
  const defaultVisible = isYourSystems
    ? sorted.slice(0, displayThreshold)
    : majorItems.length > 0
      ? majorItems
      : sorted.slice(0, displayThreshold);
    ...
    <div>
      ...
      >
        {visibleItems.map((p) => {
          ...
          if (isYourSystems && userSystemCounts) {
            const count =
              userSystemCounts.get(p.name) ||
              (p.abbreviation ? userSystemCounts.get(p.abbreviation) : 0) ||
              0;
            subtitle = `${count} ${count === 1 ? "game" : "games"}`;
          } else {
            const isRedundant =
              p.abbreviation && p.name.toLowerCase().includes(p.abbreviation.toLowerCase());
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0, color: "var(--apple-label)" }}>
              Browse Systems
            </h1>
            <p
              style={{
        ...
        {loading ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 36 }}>
            ...
            {groupedByEra.map(({ label, items }) => (
              <EraGroup key={label} label={label} items={items} navigate={navigate} />
            ))}
          </div>
        )}
      </div>
...
function SystemDirectory({
  ...
  const activeFilterCount =
    (genre ? 1 : 0) + (startYear || endYear ? 1 : 0) + (minRating ? 1 : 0) + (includeAll ? 1 : 0);
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0, color: "var(--apple-label)" }}>
              {platformName}
            </h1>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <div style={{ position: "relative", width: 200 }}>
              ...
              <input
                ...
                aria-label="Search games in this system"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                style={{
                  width: "100%",
        ...
        {activeFilterCount > 0 && !showFilters && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            ...
            {(startYear || endYear) && (
              ...
              >
                <span>
                  Years: {startYear || "any"}–{endYear || "any"}
                </span>
                <X
                  size={12}
        ...
        {selectMode && (
          <div
            style={{ display: "flex", alignItems: "center", gap: 12, paddingTop: "var(--space-1)" }}
          >
            <Button
            ...
            >
              {selectedIds.size > 0 &&
              selectedIds.size === games.filter((g) => !libraryMap.has(g.igdbId)).length
                ? "Deselect All Loaded"
                : "Select All Loaded"}
            </Button>
            <span style={{ color: "var(--apple-separator)" }}>|</span>
            <span
              style={{ fontSize: "var(--font-size-base)", color: "var(--apple-secondary-label)" }}
            >
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "var(--space-6) var(--space-8) 80px" }}>
        ...
        {hasMore && !loading && games.length > 0 && (
          ...
          >
            {loadingMore ? (
              <Loader2 size={20} className="animate-spin" color="var(--apple-tertiary-label)" />
            ) : (
              <span
                style={{ fontSize: "var(--font-size-base)", color: "var(--apple-tertiary-label)" }}
              >
                Scroll for more
              </span>
            )}
