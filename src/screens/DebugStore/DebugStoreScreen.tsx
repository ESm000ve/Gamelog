...
export function DebugStoreScreen() {
  ...
  async function runTest() {
    if (
      !window.confirm(
        "Warning: Running this diagnostic will clear your entire local library database. Continue?"
      )
    ) {
      return;
    }
    ...
    try {
      ...
      appendLog(
        `   -> Verified update. New status: ${updatedLog?.status}. Rating: ${updatedLog?.rating}`
      );
  ...
  return (
    ...
    >
      ...
      >
        Running this diagnostic check verifies database read/write operations. Note: this will clear
        existing local database data.
      </p>
      <Button
        onClick={runTest}
        disabled={isRunning}
      ...
      >
        ...
        {logs.length === 0 && (
          <div style={{ fontSize: "var(--font-size-base)", color: "var(--apple-tertiary-label)" }}>
            Click button above to start diagnostic test.
          </div>
        )}
      </div>
    </div>
