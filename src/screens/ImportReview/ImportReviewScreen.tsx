  ...
  if (loading || importing) {
    return (
      ...
      >
        ...
        />
        <h2 style={{ fontSize: "var(--font-size-xl)", fontWeight: 600, marginBottom: 10 }}>
          {importing ? "Importing to Library..." : "Analyzing Steam Library..."}
        </h2>
        <p style={{ color: "var(--apple-secondary-label)", marginBottom: "var(--space-5)" }}>
          {importing ? "Saving games to local database" : statusMsg}
        </p>
      ...
      >
        <div>
          ...
          <p style={{ color: "var(--apple-secondary-label)" }}>
            Found {matches.length} games. {highConf.length} exact matches, {lowConf.length} need
            review.
          </p>
        </div>
        <div style={{ display: "flex", gap: 20, alignItems: "center" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14 }}>
            <input
              type="checkbox"
              checked={overwritePlaytime}
              onChange={(e) => setOverwritePlaytime(e.target.checked)}
            />
        ...
        {lowConf.length > 0 && (
          <div style={{ marginBottom: "var(--space-10)" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {lowConf.map((m) => (
                <MatchRow
                  key={m.imported.sourceId}
                  match={m}
                  selected={selectedIds.has(m.imported.sourceId)}
                  onToggle={() => toggleSelect(m.imported.sourceId)}
                />
              ))}
            </div>
          </div>
  ...
  return (
    ...
    >
      <input
        ...
        readOnly
        disabled={!g}
        style={{ width: 18, height: 18, cursor: g ? "pointer" : "default" }}
      />
      ...
      {g ? (
        <>
          <div
            style={{
              ...
              height: 56,
              borderRadius: "var(--radius-sm)",
              background: g.coverUrl ? `url(${g.coverUrl}) center/cover` : "var(--apple-separator)",
              flexShrink: 0,
            }}
          />
          <div style={{ flex: 1 }}>
            ...
            >
              Steam: {match.imported.sourceName}
            </div>
          </div>
        </>
      ) : (
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 600, fontSize: 15, marginBottom: "var(--space-1)" }}>
            {match.imported.sourceName}
          </div>
          <div style={{ fontSize: "var(--font-size-base)", color: "var(--apple-red)" }}>
            No IGDB match found
          </div>
        </div>
      )}
