...
export function FilterBar({ spec, onChange }: FilterBarProps) {
  ...
  const toggleArrayItem = <K extends "status" | "genres" | "platforms" | "tagIds">(
    key: K,
    val: string
  ) => {
  ...
  return (
    <div ref={panelRef} style={{ position: "relative" }}>
      ...
      >
        <Filter
          size={14}
          color={isOpen || activeCount > 0 ? "white" : "var(--apple-tertiary-label)"}
          aria-hidden="true"
        />
      ...
      {isOpen && (
        ...
        >
          ...
          <FacetGroup label="Status">
            {STATUSES.map((s) => (
              <Checkbox
                key={s}
                label={s}
                checked={(spec.status || []).includes(s)}
                onChange={() => toggleArrayItem("status", s)}
              />
            ))}
          </FacetGroup>
          ...
          <FacetGroup label="Genres">
            {GENRES.map((g) => (
              <Checkbox
                key={g}
                label={g}
                checked={(spec.genres || []).includes(g)}
                onChange={() => toggleArrayItem("genres", g)}
              />
            ))}
          </FacetGroup>
          ...
          <FacetGroup label="Platforms">
            {PLATFORMS.map((p) => (
              <Checkbox
                ...
                onChange={() => toggleArrayItem("platforms", p)}
              />
            ))}
          </FacetGroup>
            ...
            <FacetGroup label="Tags">
              {tags.map((t) => (
                <Checkbox
                  key={t.id}
                  label={t.name}
                  checked={(spec.tagIds || []).includes(t.id)}
                  onChange={() => toggleArrayItem("tagIds", t.id)}
                />
              ))}
            </FacetGroup>
          )}
          ...
          <RangeFacet
            label="Release Year"
            min={spec.releaseYear?.min}
            max={spec.releaseYear?.max}
            onChange={(min, max) =>
              update({
                releaseYear: min === undefined && max === undefined ? undefined : { min, max },
              })
            }
            placeholderMin="1990"
            placeholderMax="2025"
          ...
          <RangeFacet
            label="Length (hours)"
            min={spec.timeToBeat?.min}
            max={spec.timeToBeat?.max}
            onChange={(min, max) =>
              update({
                timeToBeat: min === undefined && max === undefined ? undefined : { min, max },
              })
            }
            placeholderMin="0"
            placeholderMax="100"
          ...
          <RangeFacet
            ...
            max={spec.rating?.max}
            onChange={(min, max) =>
              update({ rating: min === undefined && max === undefined ? undefined : { min, max } })
            }
            placeholderMin="1"
            placeholderMax="5"
          />
    ...
    <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
      ...
      {spec.tagIds?.map((tid) => {
        ...
        return (
          <Badge
            key={tid}
            label={`Tag: ${t?.name || tid}`}
            onRemove={() => toggleArrayItem("tagIds", tid)}
          />
        );
      })}
      {spec.releaseYear && (
        <Badge
          label={`Year: ${spec.releaseYear.min || "Any"}-${spec.releaseYear.max || "Any"}`}
          onRemove={() => update({ releaseYear: undefined })}
        />
      )}
      {spec.timeToBeat && (
        <Badge
          label={`Length: ${spec.timeToBeat.min || 0}h - ${spec.timeToBeat.max || "Any"}h`}
          onRemove={() => update({ timeToBeat: undefined })}
        />
      )}
      {spec.rating && (
        <Badge
          label={`Rating: ${spec.rating.min || 0} - ${spec.rating.max || 5}`}
          onRemove={() => update({ rating: undefined })}
        />
      )}
  ...
  return (
    <div
      ...
      style={{
        ...
        fontSize: "var(--font-size-base)",
        padding: "var(--space-1) var(--space-2)",
        background: checked ? "var(--apple-accent)" : "var(--apple-fill)",
        color: checked ? "white" : "var(--apple-label)",
        borderRadius: "var(--radius-sm)",
        cursor: "pointer",
